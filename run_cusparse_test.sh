...
do
    ...
    {
        __time -v bash -c "timeout $TIME_LIMIT make icholesky i=$i && ./icholesky $entry >> result_ic.log" 2>&1
        echo "__EXIT_CODE__:$?"
    } 2>&1 | awk -F: '
    ...
    echo "Peak Memory: ${MEM} KB" >> result_ic.log
    echo "" >> result_ic.log;
    RESULT=$(check_exit_status $EXIT_CODE "icholesky")
    echo "$RESULT" >> result_ic.log;
    echo "-----------" >> result_ic.log;
    echo "" >> result_ic.log;
