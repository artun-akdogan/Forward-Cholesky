...
int main()
{
    ...
    std::vector<idx_t> m_rows_unordered;
    std::vector<idx_t> m_cols_unordered;
    std::vector<double> m_values_unordered;
    std::vector<idx_t> m_rows;
    std::vector<idx_t> m_cols;
    std::vector<double> m_values;
    ...
    m_rows_unordered.push_back(0);
    for (idx_t l = 0; l < matrix.size(); l++)
    {
        for (idx_t i = 0; i < matrix[l].size(); i++)
        {
            m_cols_unordered.push_back(matrix[l][i].column);
            m_values_unordered.push_back(matrix[l][i].data);
        }
        m_rows_unordered.push_back(m_cols_unordered.size());
    }

    std::cout << "Total nonzero " << m_cols_unordered.size() << std::endl;
    ...
    for (idx_t i = 0; i < m_rows.size() - 1; i++)
    {
        for (idx_t l = m_rows[i]; l < m_rows[i + 1]; l++)
        {
            ...
            else if (m_cols[l] > i)
            {
                ...
                for (idx_t q = m_rows[i]; q < m_rows[i + 1]; q++){
                    std::cout << m_cols[q] << " ";
                }
                std::cout << std::endl;
                std::cout << i << " "<< std::endl;
